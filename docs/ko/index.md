🌐 언어: [English](../en/index.md) | [日本語](../ja/index.md) | [한국어](../ko/index.md) | [ไทย](../th/index.md)

# AstroScope AI 천문대

AstroScope AI는 지능형 관측 계획 시스템이자 인터랙티브 플라네타륨입니다.

## 구현 완료

- 다국어 인터페이스
- 관측 위치 좌표 입력
- 현지 시간의 UTC 변환
- 율리우스일
- 수정 율리우스일
- 지방 겉보기 항성시

## 문서

- [Gaia DR3 항성 카탈로그 탐색기](gaia_explorer.md)

- [천문 관측 로그북](observation_logbook.md)

- [천체사진 센서 및 모자이크 플래너](astrophotography_planner.md)

- [망원경 및 접안렌즈 시뮬레이터](telescope_simulator.md)

- [날씨를 반영한 관측 조건](weather_conditions.md)

- [야간 타임라인과 관측 일정](night_schedule.md)

- [스마트 관측 플래너](observation_planner.md)

- [인터랙티브 현지 하늘 지도](interactive_sky_map.md)

- [태양계 탐색기](solar_system.md)

- [지평 좌표와 가시성](horizontal_visibility.md)

- [적도 좌표](equatorial_coordinates.md)

- [관측 위치와 천문 시간](observer_time.md)

## 향후 목표

- 천구 좌표 계산
- 천체 가시성 계산
- 인터랙티브 별 지도
- 망원경 시야 시뮬레이션
- 천문 카탈로그 탐색

<!-- mission-14-exoplanet-explorer -->
## Mission 14: 외계행성 트랜싯 탐색기

Mission 14는 AstroScope를 NASA Exoplanet Archive와 연결합니다.

대시보드는 카탈로그 검색, 행성 집단 그래프, 행성 검사기, CSV 내보내기, ADQL 확인, 교육용 트랜싯 시뮬레이터를 제공합니다.

온화한 지구형 후보라는 분류는 간단한 선별 결과일 뿐이며 해당 행성이 거주 가능하다는 의미가 아닙니다.
