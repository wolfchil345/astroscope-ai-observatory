"""Streamlit presentation for the AstroScope Observatory page."""
...
def render_observatory_dashboard(language: str) -> None:
    ...
    if calculate_solar_system_button:
        try:
            ...
            st.write(
                f"**{translate('ephemeris', language)}:** "
                f"{translate('builtin_ephemeris', language)}"
            )
    ...
    if generate_sky_map_button:
        if not (include_catalog_objects or include_solar_system_objects):
            ...
            try:
                ...
                visible_sky_points = tuple(
                    point for point in sky_map_points if point.is_above_horizon
                )
    ...
    if create_plan_button:
        if not (include_catalog_targets or include_solar_system_targets):
            ...
            try:
                ...
                recommended_entries = tuple(
                    entry for entry in plan_result.entries if entry.recommended
                )
    ...
    if imaging_payload is not None:
        ...
        imaging_primary_metrics[1].metric(
            ...
            (
                f"{imaging_result.field_width_degrees:.3f}° × "
                f"{imaging_result.field_height_degrees:.3f}°"
            ),
        )
        ...
        if mission12_import_button:
            if mission12_uploaded_log is None:
                ...
                try:
                    ...
                    st.session_state["mission12_location"] = (
                        mission12_imported_session.location_name
                    )

                    st.session_state["mission12_latitude"] = (
                        mission12_imported_session.latitude_degrees
                    )
                    ...
                    st.session_state["mission12_timezone"] = (
                        mission12_imported_session.timezone_name
                    )
                    ...
                    st.session_state["mission12_telescope_name"] = (
                        mission12_equipment.telescope_name
                    )
                    ...
                    st.session_state["mission12_pixel_size"] = (
                        mission12_equipment.pixel_size_um or 0.0
                    )
    ...
    if mission12_add_observation:
        try:
            ...
            mission12_existing_ids = {
                ...
                for observation in st.session_state["mission12_observations"]
            }
    ...
    def build_mission12_session() -> ObservationSession:
        ...
        mission12_filter_values = tuple(
            filter_name.strip()
            for filter_name in mission12_filters.split(",")
        ...
        mission12_equipment_snapshot = EquipmentSnapshot(
            ...
            focal_length_mm=(
                None if mission12_focal_length == 0.0 else float(mission12_focal_length)
            ),
