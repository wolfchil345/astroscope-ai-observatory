...
## Mission 14: NASA 외계행성 트랜싯 및 온화한 행성 탐색기

AstroScope는 NASA Exoplanet Archive에서 확인된 외계행성 데이터를 가져와 인터랙티브 과학 대시보드로 표시합니다.

### 주요 기능

- 발견 방법, 트랜싯 여부, 거리, 행성 반지름, 평형 온도로 검색
- 반지름-공전 주기, 질량-반지름, 온도-복사량 비교 그래프
- 궤도, 행성, 항성, 트랜싯 추정값 상세 확인
- 교육용 사다리꼴 트랜싯 광도 곡선 시뮬레이션
- 온화한 지구형 행성 후보 선별
- 검색 결과 CSV 내보내기
- 생성된 ADQL 쿼리 확인
...
온화 조건 분류는 후보를 선별하기 위한 도구이며 행성의 거주 가능성을 증명하지 않습니다. 트랜싯 확률, 밀도, 기후 관련 값에는 단순화된 계산 또는 아카이브의 보고값이 사용됩니다. 트랜싯 시뮬레이터에는 주연 감광, 항성 활동, 대기 효과, 관측 장비 잡음이 포함되지 않습니다.

### 데이터 출처

카탈로그 데이터는 NASA Exoplanet Archive TAP 서비스를 통해 `pscomppars` 테이블에서 가져옵니다.
